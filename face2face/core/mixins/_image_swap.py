...
class _ImageSwap:
    ...
    def _swap_faces(
        ...
        for target_index in range(len(target_faces)):
            ...
            if isinstance(source_face, list):
                if len(source_face) > 0:
                    source_face = source_face[0]  # Take the first Face object from the list
                else:
                    print(f"Empty source face list at index {source_index}. Skipping.")
                    continue

            if not isinstance(source_face, Face):
                print(f"Invalid source face at index {source_index}. Skipping.")
                continue

            if source_face is None:
                continue

            # if target_index < len(target_faces) and target_faces[target_index] is not None and source_face is not None:
            #     result = self._face_swapper.get(result, target_faces[target_index], source_face, paste_back=True)
            # else:
            #     print("Invalid input face data or index out of range.")
